...
export class QueryBuilder<T> {
  ...
  filter() {
    ...
    allValueFields.forEach((field) => {
      if (queryObj[field] === "all") delete queryObj[field];
    });
    ...
    Object.keys(queryObj).forEach((key) => {
      if (typeof queryObj[key] === "string" && filterableTextFields.includes(key)) {
  ...
  async countTotal() {
    ...
    const total = await (this.modelQuery.model as Model<T>).countDocuments(totalQueries);
